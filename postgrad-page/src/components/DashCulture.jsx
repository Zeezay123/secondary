...
const DashCulture = () => {
  ...
  const handleCreate = async () => {
    try {
      const response = await fetch(`/api/settings/createcult`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        setErrMsg(errorData.message || 'Failed to create excursion');
        return;
      }

      const data = await response.json();
      setSuccessMsg('Excursion created successfully!');
      setErrMsg('');
    } catch (error) {
  ...
  return (
    <div className="flex flex-col gap-3 mx-auto max-w-7xl py-12 w-3xl">
      <div className="font-bold text-black text-4xl mb-7 mx-auto">
        Enter Cultural Events Information
      </div>
      ...
      <div className="flex flex-col gap-3 mt-6">
        <Button onClick={handleCreate}  >
          Create Culture Event
        </Button>
       <Link to={'/dashboard?tab=updatecult'} className='w-full'>  <Button className='w-full'  >
         Update
        </Button></Link>
      </div>
    </div>
